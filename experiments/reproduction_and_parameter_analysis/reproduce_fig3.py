...
def main():
    ...
    base_config_path = os.path.join(script_path, 'reproduce_fig3.yaml')
    ...
    # Plot 0 jitter trajectories
    j1 = 0
    model_configs['jitter'] = j1  
    models = run_simulations(base_config_path, model_configs)
    plot_trajectory_on_ax(ax[0, 0], "A: "+param_strs[0], models[0])
    plot_trajectory_on_ax(ax[0, 1], "B: "+param_strs[1], models[1])
    
    j2 = 0.05
    model_configs['jitter'] = j2
    models2 = run_simulations(base_config_path, model_configs)
    plot_trajectory_on_ax(ax[1, 0], "", models2[0])
    plot_trajectory_on_ax(ax[1, 1], "", models2[1])
    
    j2 = 0.05
    model_configs['k'] = 0.75
    models2 = run_simulations(base_config_path, model_configs)
    plot_trajectory_on_ax(ax[2, 0], "", models2[0])
    plot_trajectory_on_ax(ax[2, 1], "", models2[1])
    
    j2 = 0.05
    k = 0.75
    model_configs['k'] = 0.75
    rho = 40
    model_configs['rho'] = rho
    models2 = run_simulations(base_config_path, model_configs)
    plot_trajectory_on_ax(ax[3, 0], "", models2[0])
    plot_trajectory_on_ax(ax[3, 1], "", models2[1])
    ...
    plt.tight_layout()
    filepath = os.path.join(script_path, 'reproduce_fig3.png')
